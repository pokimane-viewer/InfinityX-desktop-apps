...
def style_transfer_main(argv,log_fn=None,spin_fn=None,lock_fn=None):
    ...
    return curr
def _avg(img):return np.array(img.convert("L")).mean()
def _auto(img):
...
def _merge_imgs(paths,resize,rng,crop,same=False,full=False):
    ...
    img=_auto(img)
    if crop:x,y,w,h=crop;img=img.crop((x,y,x+w,y+h))
    return img
...
def _crop_dialog(root,img_path,entry_widget):
    pass
def run_gui():
    root=tk.Tk();root.title("InfinityX")
    ...
    notebook=ttk.Notebook(root);fs,fm,fp,ff,fd=(ttk.Frame(notebook)for _ in range(5))
    for f,t in zip((fs,fm,fp,ff,fd),("Style Transfer","Merge Images","Photo Edit","FFMPEG","DALL-E")):notebook.add(f,text=t)
    ...
    def download_file(path):
        if not path:return
        dst_dir=os.path.expanduser('~/Downloads')
        os.makedirs(dst_dir,exist_ok=True)
        dst=os.path.join(dst_dir,os.path.basename(path))
        shutil.copy(path,dst);log(f"Saved {dst}")
    ttk.Label(fs,text="Content:").grid(row=0,column=0,sticky="e");ent_content=ttk.Entry(fs,width=50);ent_content.grid(row=0,column=1)
    prev_content=ttk.Label(fs);prev_content.grid(row=0,column=3)
    btn_dl_style=ttk.Button(fs,text="Download",state="disabled",command=lambda:download_file(style_last_out[0]));btn_dl_style.grid(row=0,column=4,padx=4)
    ttk.Label(fs,text="Style:").grid(row=1,column=0,sticky="e");ent_style=ttk.Entry(fs,width=50);ent_style.grid(row=1,column=1)
    def _browse(entry,cb=None):
        ...
        if p:
            ...
            if cb:img=Image.open(p);img.thumbnail((300,300));photo=ImageTk.PhotoImage(img);cb(photo)
    ttk.Button(fs,text="Browse",command=lambda:_browse(ent_content,lambda ph:(prev_content.config(image=ph),setattr(prev_content,"image",ph)))).grid(row=0,column=2)
    ...
    ttk.Label(fm,text="Primary").grid(row=0,column=0,sticky="e");ent_primary=ttk.Entry(fm,width=50);ent_primary.grid(row=0,column=1)
    lbl_primary=ttk.Label(fm);lbl_primary.grid(row=0,column=3)
    btn_dl_merge=ttk.Button(fm,text="Download",state="disabled",command=lambda:download_file(merge_last_out[0]));btn_dl_merge.grid(row=0,column=4,padx=4)
    ttk.Button(fm,text="Browse",command=lambda:_browse(ent_primary,lambda ph:(lbl_primary.config(image=ph),setattr(lbl_primary,"image",ph)))).grid(row=0,column=2)
    ttk.Label(fm,text="Additional").grid(row=1,column=0,sticky="ne");txt_imgs=tk.Text(fm,width=38,height=4);txt_imgs.grid(row=1,column=1,rowspan=2)
    def add_imgs():paths=tk.filedialog.askopenfilenames();txt_imgs.insert(tk.END,'\n'.join(paths)+"\n")
    ...
    ent_mrot=ttk.Entry(optm,width=5);ent_mrot.insert(0,"0");ttk.Label(optm,text="Rotate").grid(row=0,column=0);ent_mrot.grid(row=0,column=1)
    ...
    mf=tk.StringVar(value="PNG");m_om=ttk.OptionMenu(optm,mf,"PNG","PNG","JPEG","WEBP");m_om.grid(row=2,column=4)
    def run_merge():
        def task():
    ...
    ttk.Label(fp,text="Photo").grid(row=0,column=0,sticky="e");ent_photo=ttk.Entry(fp,width=50);ent_photo.grid(row=0,column=1)
    lbl_photo=ttk.Label(fp);lbl_photo.grid(row=1,column=0,columnspan=3)
    btn_dl_photo=ttk.Button(fp,text="Download",state="disabled",command=lambda:download_file(photo_last_out[0]));btn_dl_photo.grid(row=0,column=3,padx=4)
    ttk.Button(fp,text="Browse",command=lambda:_browse(ent_photo,lambda ph:(lbl_photo.config(image=ph),setattr(lbl_photo,"image",ph)))).grid(row=0,column=2)
    ...
    def run_photo():
        def task():
            ...
            opts={"rotate":int(ent_prot.get()),"flip":pflip.get(),"grayscale":pgray.get(),
                  "crop":ent_pcrop.get()if ent_pcrop.get()else None,"remove_bg":prem.get()}
    ...
    def run_ff():
        ...
        if not outp:
            ...
            outp=base+"_out.mp4"if cmd.get()=="Convert MP4"else base+"_audio.mp3"if cmd.get()=="Extract Audio"else base+"_resized.mp4"
    ...
    def start_spin():
        ...
        def _rot():
            if spinning[0]:
                ...
                root.after(100,_rot)
        _rot()
    def stop_spin():spinning[0]=False;spin_lbl.config(text="")
    def poll_log():
        try:
            while True:
    ...
    def prev_img():dalle_idx[0]=(dalle_idx[0]-1)%len(dalle_imgs);show_img(dalle_idx[0])
    def next_img():dalle_idx[0]=(dalle_idx[0]+1)%len(dalle_imgs);show_img(dalle_idx[0])
